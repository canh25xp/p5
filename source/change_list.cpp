...
ChangeList::ChangeList(const std::string &clNumber, const std::string &clDescription, const std::string &userID, const int64_t &clTimestamp)
    : number(clNumber), user(userID), description(clDescription), timestamp(clTimestamp), changedFileGroups(ChangedFileGroups::Empty()), stateCV(new std::condition_variable()), stateMutex(new std::mutex()), filesDownloaded(-1), state(Initialized) {
}

void ChangeList::PrepareDownload(const BranchSet &branchSet) {
    ChangeList &cl = *this;

    ThreadPool::GetSingleton()->AddJob([&cl, &branchSet](P5 *p4) {
...
void ChangeList::StartDownload(const int &printBatch) {
    ...
    ThreadPool::GetSingleton()->AddJob([&cl, printBatch](P5 *p4) {
        // Wait for describe to finish, if it is still running
        {
            std::unique_lock<std::mutex> lock(*cl.stateMutex);
            cl.stateCV->wait(lock, [&cl]() { return cl.state == Described; });
        }
...
void ChangeList::Flush(std::shared_ptr<std::vector<std::string>> printBatchFiles, std::shared_ptr<std::vector<FileData *>> printBatchFileData) {
    ...
    ThreadPool::GetSingleton()->AddJob([this, printBatchFiles, printBatchFileData](P5 *p4) {
...
void ChangeList::WaitForDownload() {
    ...
    stateCV->wait(lock, [this]() { return state == Downloaded; });
}
