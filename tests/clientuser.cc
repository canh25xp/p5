...
void ClientUser::Prompt(Error *err, StrBuf &buf, int noEcho, Error *e) {
    StrBuf ebuf;
    err->Fmt(ebuf, EF_PLAIN);
    Prompt(ebuf, buf, noEcho, e);
}

void ClientUser::Prompt(Error *err, StrBuf &buf, int noEcho, int noOutput, Error *e) {
    StrBuf ebuf;
    err->Fmt(ebuf, EF_PLAIN);
    Prompt(ebuf, buf, noEcho, noOutput, e);
}

void ClientUser::Prompt(const StrPtr &msg, StrBuf &buf, int noEcho, Error *e) {
    Prompt(msg, buf, noEcho, 0, e);
}

void ClientUser::Prompt(const StrPtr &msg, StrBuf &buf, int noEcho, int noOutput, Error *e) {
    if (!noOutput)
        printf("%s", msg.Text());
...
void ClientUser::Help(const char *const *help) {
    for (; *help; help++)
        printf("%s\n", *help);
}

void ClientUser::HandleError(Error *err) {
...
void ClientUser::OutputInfo(char level, const char *data) {
    if (quiet)
        return;
...
void ClientUser::OutputText(const char *data, int length) {
...
void ClientUser::OutputBinary(const char *data, int length) {
#ifdef OS_NT
...
void ClientUser::InputData(StrBuf *buf, Error *) {
    if (commandChaining) {
        for (;;) {
            StrBuf lb;
...
void ClientUser::ErrorPause(char *errBuf, Error *e) {
    StrBuf buf;
    OutputError(errBuf);
...
void ClientUser::HandleUrl(const StrPtr *url) {
    StrBuf result;
    Error e;
...
void ClientUser::Edit(FileSys *f1, Error *e) {
    Edit(f1, enviro, e);
...
void ClientUser::Edit(FileSys *f1, Enviro *env, Error *e) {
...
void ClientUser::Diff(FileSys *f1, FileSys *f2, int doPage, char *df, Error *e) {
    Diff(f1, f2, NULL, doPage, df, e);
}

void ClientUser::Diff(FileSys *f1, FileSys *f2, FileSys *fout, int doPage, char *df, Error *e) {
        ...
        if (!e->Test()) {
            ...
            if (!fileError || flags.type == DiffFlags::Unified) {
                if (fout) {
                    ...
                    t = File(FileSysType((f1->GetType() & FST_L_MASK) | FST_UNICODE));
                    t->SetDeleteOnClose();
                    t->MakeGlobalTemp();
                    d.SetOutput(t->Name(), e);
                } else {
                ...
                if (output_translate) {
                    ...
                    if (pager) {
                        ...
                        if (!e->Test()) {
                            ...
                            while ((i = t->Read(buf, sizeof(buf), e)) > 0 && !e->Test())
...
void ClientUser::Merge(
    ...
    if (result->IsUnicode()) {
        ...
        if (cs != 0 && ((ext && (merger = enviro->Get(mue.Text()))) ||
...
int ClientUser::ProgressIndicator() {
    return 0;
}
...
int ClientUserProgress::ProgressIndicator() {
    return 1;
}
...
int ClientUser::Resolve(ClientMerge *m, Error *e) {
    // pun from ClientMergeStatus
    return (int)m->Resolve(e);
}

int ClientUser::Resolve(ClientResolveA *r, int preview, Error *e) {
    // pun from ClientMergeStatus
    return (int)r->Resolve(preview, e);
}

void ClientUser::Message(Error *err) {
    int keepfile = 0;
...
void ClientUser::SetOutputCharset(int charset) {
    outputCharset = charset;
}

void ClientUser::DisableTmpCleanup() {
    signaler.Disable();
}

void ClientUser::SetTransfer(ClientTransfer *t) {
    transfer = t;
}

void ClientUser::SetSSOHandler(ClientSSO *s) {
    ssoHandler = s;
}

void ClientUser::SetQuiet() {
    quiet = 1;
}

int ClientUser::CanAutoLoginPrompt() {
...
int ClientUser::IsOutputTaggedWithErrorLevel() {
    return outputTaggedWithErrorLevel;
}

void ClientUser::SetEnviro(Enviro *env) {
#ifdef HAS_CPP11
...
void ClientUser::SetVarList(StrDict *l) {
#ifdef HAS_CPP11
    std::lock_guard<std::mutex> lock(*(std::mutex *)setterGuard);
#endif
    varList = l;
